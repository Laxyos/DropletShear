...
outputfolder = "%s/post/velocity" %inputpath
...
for snap in range(nsnaps):
    ...
    if not os.path.exists(inputfile):
       ...
       # print("The input file \"%s\" could not be found!" % inputfile)
        ...
        # --------------------------------------------------------------------
        outputfile = "%s/velocity-%8.8d.png" %(outputfolder, snapindex)
        if os.path.exists(outputfile):
            ...
            if (len(segments) == 0):
                ...
                cntrl1 = ax.imshow(vel, cmap="Blues", interpolation='bilinear', origin='lower', extent=[xminp, xmaxp, yminp, ymaxp], vmax = 1.0, vmin = 0.0)
                
                l, b, w, h = ax.get_position().bounds
                cb1 = fig.add_axes([l+0.05*w, b-0.05, 0.90*w, 0.03])
                c1 = plt.colorbar(cntrl1,cax=cb1,orientation='horizontal')
                c1.set_label('$\log_{10}\left(\|u/U|\\right)$',fontsize=TickLabel, labelpad=5)
                ...
                ax.set_title('Velocity (tU/a = %4.2f)' % time, fontsize=TickLabel)
